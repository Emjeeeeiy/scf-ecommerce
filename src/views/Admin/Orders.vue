<template>
  <AdminPanelLayout subtitle="Manage and track customer order fulfillment">
    <div class="max-w-7xl mx-auto space-y-8">
      ...
      <section class="space-y-6">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div 
            ...
            :key="summary.label"
            class="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm transition-all hover:shadow-md"
          >
            <p class="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1">{{ summary.label }}</p>
            <div class="flex items-baseline gap-2">
              <span class="text-2xl font-bold text-slate-900 dark:text-white">{{ summary.value }}</span>
              <span class="text-[10px] font-medium text-slate-400 dark:text-slate-500">orders</span>
            </div>
          </div>
        </div>
        ...
        <!-- Filter Bar -->
        <div class="space-y-4">
          <div class="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col lg:flex-row items-center gap-4">
            <div class="relative flex-1 w-full">
              <Search class="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500" :size="18" />
              <input 
                v-model="searchQuery"
                type="text" 
                placeholder="Search by ID, name, email, or reference..." 
                class="w-full pl-12 pr-10 py-3 rounded-xl border-none bg-slate-50 dark:bg-slate-800/50 text-sm font-medium text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:ring-2 focus:ring-amber-400/20 transition-all"
              >
              <button 
                v-if="searchQuery" 
                @click="searchQuery = ''"
            ...
            <div class="flex items-center gap-3 w-full lg:w-auto">
              <div class="flex items-center bg-slate-50 dark:bg-slate-800/50 rounded-xl px-3 py-1.5 border border-transparent focus-within:border-slate-200 dark:focus-within:border-slate-700 transition-all flex-1 lg:flex-none">
                <Calendar :size="14" class="text-slate-400 dark:text-slate-500 mr-2" />
                <input 
                  v-model="startDate"
                  type="date" 
                  class="bg-transparent border-none p-0 text-xs font-bold text-slate-700 dark:text-slate-300 focus:ring-0"
                >
              ...
              <button 
                ...
                @click="resetFilters"
                class="p-3 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700 hover:text-slate-700 dark:hover:text-slate-200 transition-all"
                title="Reset Filters"
              >
                <RotateCcw :size="18" />
              </button>
      ...
      <section class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden">
        <div class="overflow-x-auto">
          <table class="w-full text-left border-collapse border-spacing-0">
            <thead>
              <tr class="bg-slate-50/50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800">
                ...
                <th class="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500 whitespace-nowrap text-right">Total</th>
                <th class="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500 whitespace-nowrap text-center">Action</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-slate-50 dark:divide-slate-800">
              ...
              >
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="max-w-50">
                    <div class="flex items-center gap-2 mb-0.5">
                      ...
                      <span v-if="order.isGuest" class="text-[8px] font-black uppercase tracking-tighter px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500 border border-slate-200 dark:border-slate-700">Guest</span>
                    ...
                    <p class="text-[10px] font-medium text-slate-400 dark:text-slate-500 truncate">{{ order.customerDetails?.email }}</p>
                  </div>
                </td>
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <p class="text-xs font-semibold text-slate-400 dark:text-slate-500">{{ formatDate(order.createdAt, true) }}</p>
                </td>
          ...
          <div v-if="!filteredOrders.length" class="flex flex-col items-center justify-center py-20 text-center px-6">
            <div class="w-12 h-12 bg-slate-50 dark:bg-slate-800 rounded-full flex items-center justify-center text-slate-200 dark:text-slate-700 mb-3">
              <PackageSearch :size="24" />
            </div>
            <p class="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">No matching orders found</p>
          </div>
        </div>
      </section>
    </div>
    ...
    >
      <div v-if="selectedOrder" class="fixed inset-0 z-100 flex items-center justify-center p-4 backdrop-blur-md">
        ...
        <div class="relative w-full max-w-4xl max-h-[90vh] overflow-hidden rounded-[2.5rem] bg-white dark:bg-slate-900 shadow-2xl flex flex-col">
          <!-- Modal Header -->
          <div class="px-8 py-6 border-b border-slate-50 dark:border-slate-800 flex items-center justify-between bg-slate-50/30 dark:bg-slate-800/30 shrink-0">
            <div class="flex items-center gap-4">
              <div class="bg-slate-900 dark:bg-amber-400 text-white dark:text-slate-950 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-tighter">
                #{{ selectedOrder.id.slice(0, 12) }}
              </div>
              <span v-if="selectedOrder.isGuest" class="px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border bg-slate-50 dark:bg-slate-800 text-slate-500 dark:text-slate-400 border-slate-200 dark:border-slate-700">
                Guest Order
              </span>
            ...
            <div class="grid lg:grid-cols-2 gap-10">
              ...
              <div class="space-y-8">
                <div>
                  ...
                  <div class="bg-slate-50 dark:bg-slate-800/50 p-6 rounded-3xl border border-slate-100 dark:border-slate-800 space-y-4">
                    <div class="flex items-center gap-4">
                      <div class="w-12 h-12 rounded-2xl bg-white dark:bg-slate-900 shadow-sm flex items-center justify-center text-slate-400 dark:text-slate-600 border border-slate-100 dark:border-slate-800">
                        <User :size="24" />
                      </div>
                      <div>
                        ...
                        <p class="text-xs font-medium text-slate-500 dark:text-slate-400">{{ selectedOrder.customerDetails?.email }}</p>
                      </div>
                    </div>
                    <div class="pt-4 border-t border-slate-200/50 dark:border-slate-700 space-y-3">
                      <div class="flex items-center gap-3">
                        <Phone :size="14" class="text-slate-400 dark:text-slate-500" />
                        <span class="text-xs font-bold text-slate-700 dark:text-slate-300">{{ selectedOrder.customerDetails?.contactNo || selectedOrder.customerDetails?.contact }}</span>
                      </div>
                      <div class="flex items-start gap-3">
                        <MapPin :size="14" class="text-slate-400 dark:text-slate-500 mt-0.5" />
                        <span class="text-xs font-medium text-slate-600 dark:text-slate-400 leading-relaxed">{{ selectedOrder.customerDetails?.completeAddress }}</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div class="bg-slate-50 dark:bg-slate-800/50 p-6 rounded-3xl border border-slate-100 dark:border-slate-800 space-y-4">
                    <div class="flex items-center justify-between">
                      <span class="text-xs font-bold text-slate-500 dark:text-slate-400">Method</span>
                    ...
                    <!-- GCash Info -->
                    <div v-if="selectedOrder.paymentMethod === 'gcash'" class="pt-4 border-t border-slate-200/50 dark:border-slate-700 space-y-4">
                      <div>
                        <p class="text-[8px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1">Reference Number</p>
                      ...
                      <div v-if="selectedOrder.receiptUrl" class="space-y-2">
                        <p class="text-[8px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Receipt Screenshot</p>
                        <div class="relative group aspect-video rounded-2xl overflow-hidden border-2 border-white dark:border-slate-800 shadow-md bg-white dark:bg-slate-900 cursor-pointer" @click="viewFullImage(selectedOrder.receiptUrl)">
              ...
              <div class="space-y-8">
                <div>
                  <div class="flex items-center justify-between mb-4">
                    <h3 class="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest flex items-center gap-2">
                      <Package :size="12" /> Order Manifest
                    </h3>
                    <span class="text-[10px] font-bold text-slate-400 dark:text-slate-500">{{ selectedOrder.items?.length || 0 }} items</span>
                  </div>
                  <div class="space-y-3">
                    <div 
                    ...
                    >
                      <div class="min-w-0 pr-4">
                        ...
                        <p class="text-[10px] font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-tight mt-0.5">
                          {{ item.color }} • {{ item.size }}
                        </p>
                      </div>
                      <div class="text-right shrink-0">
                        <p class="text-xs font-black text-slate-900 dark:text-white">x{{ item.quantity }}</p>
                        <p class="text-[10px] font-medium text-slate-500 dark:text-slate-400">{{ formatCurrency(item.priceAtPurchase) }}</p>
                      </div>
                    </div>
                  </div>
                  <div class="mt-6 pt-6 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between px-2">
                    <span class="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">Grand Total</span>
                ...
                <div>
                  ...
                  <div class="bg-slate-900 dark:bg-amber-400 p-6 rounded-3xl text-white dark:text-slate-950 space-y-4">
                    <div>
                      <p class="text-[9px] font-bold text-slate-400 dark:text-slate-700 uppercase tracking-widest mb-2">Update Order Status</p>
                      <select
                        class="w-full rounded-xl bg-white/10 dark:bg-black/10 border-none px-4 py-3 text-xs font-bold text-white dark:text-slate-950 focus:ring-2 focus:ring-amber-400 dark:focus:ring-slate-950 transition-all cursor-pointer"
                    ...
                    <p class="text-[8px] font-medium text-slate-400 dark:text-slate-700 leading-relaxed italic">
