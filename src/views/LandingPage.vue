<template>
  <AppShell>
    <div class="space-y-10 pb-16 sm:space-y-16">
      <!-- Hero Section -->
     <section 
        class="relative overflow-hidden rounded-3xl bg-neutral-900 px-6 py-12 text-center text-white sm:px-10 sm:py-20"
      ...
      <section class="space-y-6">
        <div class="flex flex-col items-end justify-between gap-3 sm:flex-row">
          <div class="w-full text-left">
             ...
             <h2 class="text-2xl font-black tracking-tight text-neutral-900 sm:text-3xl">Latest Arrivals</h2>
          </div>
          <router-link to="/shop" class="group flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-neutral-400 transition-colors hover:text-neutral-900">
            View All Collection
        ...
        <div v-if="loading" class="flex gap-4 overflow-x-auto pb-4 lg:grid lg:grid-cols-6 lg:overflow-visible lg:pb-0 scrollbar-hide">
          <div v-for="i in 6" :key="i" class="min-w-40 flex-1 animate-pulse space-y-3 lg:min-w-0">
            <div class="aspect-4/5 rounded-2xl bg-neutral-100"></div>
        ...
        <div v-else-if="latestProducts.length > 0" class="flex gap-4 overflow-x-auto pb-4 lg:grid lg:grid-cols-6 lg:overflow-visible lg:pb-0 scrollbar-hide">
          ...
          >
            <div class="aspect-4/5 overflow-hidden rounded-2xl bg-white p-1.5 ring-1 ring-neutral-100 transition-all duration-500">
              <div class="relative h-full w-full overflow-hidden rounded-xl bg-neutral-50 shadow-inner">
                <img
            ...
            <div class="mt-3 text-center sm:text-left">
              <h3 class="text-[10px] font-black uppercase tracking-widest text-neutral-900 line-clamp-1 group-hover:text-amber-500 transition-colors">{{ product.name }}</h3>
              <!-- <p class="mt-0.5 text-xs font-bold text-neutral-400">{{ formatCurrency(product.basePrice) }}</p> -->
            </div>
          </router-link>
        </div>
      </section>
      ...
      <section class="grid gap-8 lg:grid-cols-2 lg:items-center">
        <div class="space-y-4">
          <div class="rounded-2xl bg-white p-6 border border-neutral-300 ring-1 ring-neutral-100 sm:p-8">
            <div class="mb-4 flex h-10 w-10 items-center justify-center rounded-xl bg-amber-50 text-amber-500">
              <Target :size="20" />
            </div>
            <h2 class="text-xl font-black tracking-tight text-neutral-900 sm:text-2xl">
              {{ settings.mission.title }}
            </h2>
            <p class="mt-3 text-sm leading-relaxed text-neutral-500 sm:text-base">
              {{ settings.mission.description }}
            </p>
          </div>
          <div class="rounded-2xl bg-neutral-900 p-6  sm:p-8">
        ...
        <div class="relative rounded-3xl bg-neutral-200 aspect-square overflow-hidden flex items-center justify-center">
            <!-- Background Image -->
            <img 
                src="/scfphoto.jpg" 
                alt="SCF Photo" 
      ...
      <!-- Contact Section -->
      <section class="mx-auto max-w-5xl rounded-3xl bg-white p-6 border border-neutral-300 ring-1 ring-neutral-100 sm:p-12">
        <div class="grid gap-10 md:grid-cols-2">
          <div class="space-y-6">
            <div>
              <p class="text-[9px] font-black uppercase tracking-[0.3em] text-amber-500 mb-1.5">Connect</p>
              <h2 class="text-2xl font-black tracking-tight text-neutral-900 sm:text-3xl">Get in Touch</h2>
              <p class="mt-3 text-xs leading-relaxed text-neutral-500 sm:text-sm">
            ...
            <div class="space-y-4">
              <div class="flex items-center gap-4">
                <div class="flex h-10 w-10 items-center justify-center rounded-xl bg-neutral-50 text-neutral-400 transition-colors hover:bg-neutral-900 hover:text-white">
                  <Mail :size="18" />
                </div>
                <div>
                   <p class="text-[7px] font-black uppercase tracking-widest text-neutral-400">Email Us</p>
                   <span class="text-neutral-900 font-black text-xs sm:text-sm">{{ settings.contact.email }}</span>
                </div>
              </div>
              <div class="flex items-center gap-4">
                <div class="flex h-10 w-10 items-center justify-center rounded-xl bg-neutral-50 text-neutral-400 transition-colors hover:bg-neutral-900 hover:text-white">
                ...
                <div>
                   <p class="text-[7px] font-black uppercase tracking-widest text-neutral-400">Visit Us</p>
                   <span class="text-neutral-900 font-black text-xs sm:text-sm">{{ settings.contact.address }}</span>
                </div>
              </div>
              <a 
              ...
              >
                <div class="flex h-10 w-10 items-center justify-center rounded-xl bg-neutral-50 text-neutral-400 transition-colors group-hover:bg-blue-600 group-hover:text-white">
                  <Facebook :size="18" />
                </div>
                <div>
                   <p class="text-[7px] font-black uppercase tracking-widest text-neutral-400 group-hover:text-blue-600 transition-colors">Follow Us</p>
                   <span class="text-neutral-900 font-black text-xs sm:text-sm group-hover:text-blue-700 transition-colors">Facebook Page</span>
                </div>
              </a>
            </div>
          </div>

          <form @submit.prevent class="flex flex-col gap-3 rounded-2xl bg-neutral-50 p-5 sm:p-6">
            <input type="text" placeholder="Full Name" class="rounded-xl border-none bg-white p-4 text-xs font-bold text-neutral-900 shadow-sm outline-none ring-1 ring-neutral-100 focus:ring-2 focus:ring-amber-400 transition-all" />
            <input type="email" placeholder="Email Address" class="rounded-xl border-none bg-white p-4 text-xs font-bold text-neutral-900 shadow-sm outline-none ring-1 ring-neutral-100 focus:ring-2 focus:ring-amber-400 transition-all" />
            <textarea placeholder="Your Message" rows="3" class="rounded-xl border-none bg-white p-4 text-xs font-bold text-neutral-900 shadow-sm outline-none ring-1 ring-neutral-100 focus:ring-2 focus:ring-amber-400 transition-all"></textarea>
            <button class="mt-3 rounded-xl bg-neutral-900 py-4 text-[10px] font-black uppercase tracking-widest text-white transition-all hover:bg-neutral-800 hover:shadow-xl active:scale-95 shadow-lg shadow-neutral-900/20">
              Send Message
            </button>
          </form>
        </div>
